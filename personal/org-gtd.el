...
(use-package org-gtd
  ...
  (org-edna-load)
  :bind
  (("C-c h c" . org-gtd-capture) ;; add item to inbox
  ("C-c h a" . org-agenda-list) ;; see what's on your plate today
  ("C-c h p" . org-gtd-process-inbox) ;; process entire inbox
  ("C-c h n" . org-gtd-show-all-next) ;; see all NEXT items
  ("C-c h s" . org-gtd-show-stuck-projects)) ;; see projects that don't have a NEXT item
  :init
  (bind-key "C-c h f" 'org-gtd-clarify-finalize)) ;; the keybinding to hit when you're done editing an item in the processing phase
